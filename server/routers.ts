...
export const appRouter = router({
  ...
  // SCA Consultation Router
  sca: router({
    scoreConsultation: protectedProcedure
      .input(z.object({
        caseId: z.number(),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const { getDb } = await import("./db");
        const { scaConsultations } = await import("../drizzle/schema");
        
        const feedbackPrompt = `Evaluate SCA consultation across 3 domains:
1. Data Gathering (0-40)
2. Clinical Reasoning (0-40)
3. Communication (0-20)
Transcript: ${input.transcript.map(m => `${m.role}: ${m.content}`).join('\n')}
Return JSON: {domain1Score, domain2Score, domain3Score, feedback}`;

        const response = await invokeLLM({
          messages: [
            { role: "system", content: "You are a medical educator. Provide JSON feedback." },
            { role: "user", content: feedbackPrompt },
          ],
        });

        const feedbackContent = response.choices[0]?.message?.content;
        const feedbackText = typeof feedbackContent === "string" ? feedbackContent : JSON.stringify(feedbackContent);
        const feedbackData = JSON.parse(feedbackText || "{}");
        const totalScore = (feedbackData.domain1Score || 0) + (feedbackData.domain2Score || 0) + (feedbackData.domain3Score || 0);
        const passed = totalScore >= 70;

        const db = await getDb();
        if (!db) throw new Error("Database not available");

        const result = await db.insert(scaConsultations).values({
          userId: ctx.user.id,
          caseId: input.caseId,
          transcript: input.transcript,
          duration: input.duration,
          domain1Score: feedbackData.domain1Score || 0,
          domain2Score: feedbackData.domain2Score || 0,
          domain3Score: feedbackData.domain3Score || 0,
          totalScore,
          passed,
        ...
        return {
          consultationId: (result as any).insertId || 0,
          ...
          domain1Score: feedbackData.domain1Score,
          domain2Score: feedbackData.domain2Score,
          domain3Score: feedbackData.domain3Score,
          totalScore,
          passed,
    ...
    getConsultationHistory: protectedProcedure
      .query(async ({ ctx }) => {
        ...
        const db = await getDb();
        if (!db) throw new Error("Database not available");
        return await db.select().from(scaConsultations).where(eq(scaConsultations.userId, ctx.user.id));
      }),
  }),
  ...
  note360: router({
    ...
    addAnnotation: protectedProcedure
      .input(z.object({
        ...
        highlightColor: z.enum(["yellow", "green", "red", "blue", "purple"]).default("yellow"),
      }))
      .mutation(async ({ ctx, input }) => {
        const { getDb } = await import("./db");
        const db = await getDb();
        if (!db) throw new Error("Database not available");
        const { noteAnnotations } = await import("../drizzle/schema");
        return await db.insert(noteAnnotations).values({
          userId: ctx.user.id,
          noteId: input.noteId,
    ...
    getAnnotations: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        return await db.select().from(noteAnnotations).where(
          and(
            eq(noteAnnotations.userId, ctx.user.id),
            eq(noteAnnotations.noteId, input)
          )
        );
      }),
    deleteAnnotation: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const { noteAnnotations } = await import("../drizzle/schema");
        const { eq, and } = await import("drizzle-orm");
        return await db.delete(noteAnnotations).where(
          and(
            eq(noteAnnotations.id, input),
            eq(noteAnnotations.userId, ctx.user.id)
          )
        );
      }),
